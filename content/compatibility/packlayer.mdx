---
description: LayerPack optimizes resource pack distribution on proxy networks by preventing duplicate pack sends.
---

# LayerPack - Proxy Resource Pack Optimization

LayerPack is a **Velocity and BungeeCord proxy plugin** that optimizes resource pack distribution on multi-server networks.
...
LayerPack intercepts outgoing `ResourcePackSend` packets at the proxy level and **cancels duplicates**. If a player already has a pack with a matching hash, the packet is dropped, making server switching instant.

## Installation

1. Download the latest `layerpack-X.X.X.jar` from [GitHub Releases](https://github.com/oraxen/LayerPack/releases)
...
LayerPack generates a `config.yml` in its data folder with sensible defaults:
...
| `NEVER_SKIP` | Effectively disables LayerPack. |

## Server Filtering

You can limit LayerPack to specific backend servers:
...
All commands require the `layerpack.admin` permission.

| Command | Description |
|---------|-------------|
| `/layerpack reload` | Reload configuration |
| `/layerpack stats` | Show skip statistics |
| `/layerpack clear <player\|*>` | Clear pack cache for player(s) |
| `/layerpack info <player>` | Show cached pack info for a player |
| `/layerpack debug` | Show debug mode status |
...
1. Install LayerPack on your proxy
...
3. Make sure LayerPack is on the **proxy**, not backend servers

### Players seeing old pack version

Clear their pack cache:
```
/layerpack clear PlayerName
```
...
LayerPack is designed to be lightweight:
...
LayerPack uses [PacketEvents](https://github.com/retrooper/packetevents) for packet interception, which provides broad version compatibility.
